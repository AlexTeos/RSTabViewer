...
public:
    ...
    void MakeKey(unsigned char const* key,
                 unsigned char const* chain,
                 int                  keylength = DEFAULT_BLOCK_SIZE,
                 int                  blockSize = DEFAULT_BLOCK_SIZE);

private:
    //Auxiliary Function
    void Xor(unsigned char* buff, unsigned char const* chain)
    {
        if (false == m_bKeyInit)
        {
            throw exception();
    ...
    void DefEncryptBlock(unsigned char const* in, unsigned char* result);

    //Convenience method to decrypt exactly one block of plaintext, assuming
    //Rijndael's default block size (128-bit).
    // in         - The ciphertext.
    // result     - The plaintext generated from a ciphertext using the session key.
    void DefDecryptBlock(unsigned char const* in, unsigned char* result);

public:
    //Encrypt exactly one block of plaintext.
    // in           - The plaintext.
    // result       - The ciphertext generated from a plaintext using the key.
    void EncryptBlock(unsigned char const* in, unsigned char* result);

    //Decrypt exactly one block of ciphertext.
    // in         - The ciphertext.
    // result     - The plaintext generated from a ciphertext using the session key.
    void DecryptBlock(unsigned char const* in, unsigned char* result);

    void Encrypt(unsigned char const* in, unsigned char* result, size_t n, int iMode = ECB);

    void Decrypt(unsigned char const* in, unsigned char* result, size_t n, int iMode = ECB);
...
private:
    ...
    int m_iROUNDS;
    //Chain Block
    unsigned char m_chain0[MAX_BLOCK_SIZE];
    unsigned char m_chain[MAX_BLOCK_SIZE];
